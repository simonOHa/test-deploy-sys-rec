...
class IntrusionTestTIModel(db.Model):
    ...
    result_candidate_id = db.Column(db.ARRAY(db.String))
    ...
    def save_to_db(self, result, email):
        ...
        result['score'] = self._calculate_score(result_candidate_ids=[k for k in result['candidate_id']])

        if user_session:
            self._update(user_session, result)
        else:
            self.result_candidate_id = [k for k in result['candidate_id']]
    ...
    def _update(self, session, result):
        try:
            session.result_candidate_id = [k for k in result['candidate_id']]
    ...
    def read_ti_data(self):
        ...
        for index in range(1, len(d) + 1):
            ...
            text = eval(d[index]['text'].to_list()[0])
            candidates = []
            for i in range(0, len(d[index]['topic_id'])):
                candidate_id = d[index]['topic_id'].to_list()[i]
                candidate_value = d[index]['top_terms'].to_list()[i]
                candidates.append({'candidate_id': str(candidate_id), 'candidate_value': candidate_value})
            json_list.append({'question': question_id,'text': text, 'candidates': candidates})

        return json_list

    def _calculate_score(self, result_candidate_ids):
        df = pd.read_csv(self._ti_file_path)
        df = df[['question', 'good_answer']].drop_duplicates(subset=None, keep='first', inplace=False)
        total_good_answer = 0
        for index, expected in enumerate(df['good_answer'].to_list()):
            if result_candidate_ids[index] == expected:
                total_good_answer += 1

        return total_good_answer/len(df['good_answer'].to_list())
...
class IntrusionTestWIModel(db.Model):
    ...
    def read_wi_data(self):
        ...
        for index in range(1, len(d) + 1):
            question_id = d[index]['question'].to_list()[0]
            json_list[question_id] = {'question': question_id, 'candidates': d[index]['candidate'].to_list()}
